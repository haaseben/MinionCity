...
void CChildView::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    {
		if (!mGivePotionActive && !mGiveBananaActive)
		{
...
/** Handler for keeping track of mouse clicks*/
...
/** Menu handler that  makes a minion tile*/
...
/** Menu handler that gives a minion a banana*/
void CChildView::OnMinionBanana()
{
	mGiveBananaActive = !mGiveBananaActive;

}

/** Update that checks and disables Banana
*\param pCmdUI update handler
*/
void CChildView::OnUpdateMinionBanana(CCmdUI *pCmdUI)
{
	CMinionCounter visitor;
	mCity.Accept(&visitor);
	int cnt = visitor.GetNumMinions();

	if (cnt == 0)
	{
		pCmdUI->Enable(false); //disable the menu option
	}

	pCmdUI->SetCheck(mGiveBananaActive);	// Will check the menu option
}



/** Menu handler that gives a minion a potion*/
void CChildView::OnMinionPotion()
{
	mGivePotionActive = !mGivePotionActive;
}

/** Update that checks and disables Potion
* \param pCmdUI update handler
*/
void CChildView::OnUpdateMinionPotion(CCmdUI *pCmdUI)
{
	
	CMutantCounter visitor;
	mCity.Accept(&visitor);
	int cnt = visitor.GetNumMutants();
	...
	pCmdUI->SetCheck(mGivePotionActive);	// Will check the menu option
	
}

/** Menu handler that makes minions run*/
void CChildView::OnMinionRunaway()
{
	mMakeRunAwayActive = !mMakeRunAwayActive;
		CMinionTileVisitor visitor;
		mCity.Accept(&visitor);
		Invalidate();
	
}

/** Update that checks and disables RunAway
*\param pCmdUI update handler
*/
...
/** Update that checks and disables MouseTracks
*\param pCmdUI update handler
*/
